...
namespace StudentRegSys
{
    public partial class Professor : MaterialForm
    {
        ...
        private void cmbxSYear_SelectedIndexChanged(object sender, EventArgs e)//student filtering
        {
            if (cmbxModule.SelectedIndex > -1) {
                string x = cmbxModule.Items[cmbxModule.SelectedIndex].ToString();
                SqlDataAdapter adt = new SqlDataAdapter("select * from Student where Module1='"+x+ "' OR Module2='" + x + "' OR Module3='" + x + "' OR Module4='" + x + "' OR Module5='" + x + "'", con);
                DataTable dt = new DataTable();
                try
                {
                    con.Open();
        ...
        private void datagridModule_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
